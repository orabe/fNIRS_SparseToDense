...
def build_subject_grid(subjects, subject_cols):
    ...
    fig, axes = plt.subplots(n_rows, subject_cols, figsize=(4 * subject_cols, 3 * n_rows), dpi=400)
...
INCLUDE = {
    "BallSqueezingHD_modified": [
      "CNN2D_BaselineV2_lr1e-5__20251215_205904",
    # #   "CNN2D_BaselineV2_lr1e-5__20251216_015635", # channel space
    ],
        
    #     # --------------- FreshMotor ---------------
    "BallSqueezingHD_modified__FreshMotor": [
        "CNN2D_BaselineV2_lr3e-5__aug20%__20251215_210113",
        "CNN2D_BaselineV2_lr3e-5__aug100%__20251215_210222",
    ],
    # "FreshMotor": [
    #     "CNN2D_BaselineV2_lr1e-5__20251215_210007",
    #     # "CNN2D_BaselineV2_lr1e-5__20251215_211442", # channel space
    # ],
}
...
# Base color to derive gradients from (a strong blue)
GRADIENT_LEGEND_BASE_COLOR = (0.0, 0.45, 0.9)
...
def split_color_for_combo(
    ...
    return adjust_lightness(base_color, SPLIT_GRADIENT_FACTORS[split_key])
...
def plot_across_subject_boxplots(
    ...
    for m_idx, key in enumerate(all_keys):
        ...
        base_color = model_colors.get(key, GRADIENT_LEGEND_BASE_COLOR)
    ...
    # Only show dataset/model legend items, not split shade items
    legend_items: List[object] = list(handles.values())
...
def plot_across_subject_bars(
    ...
    fig, ax = plt.subplots(figsize=(8, 6))
    dataset_handles: Dict[str, Line2D] = {}
    ...
    for m_idx, key in enumerate(all_keys):
        ...
        # Use the same color for all splits for a given dataset
        dataset_color = model_colors.get(key, GRADIENT_LEGEND_BASE_COLOR)
        split_colors = [dataset_color] * len(SPLIT_ORDER)
        bars = ax.bar(
            x + offset,
            means,
            width=width * 0.95,
        ...
        dataset_label = f"{key[0]} | {key[1]}"
        if dataset_label not in dataset_handles:
            dataset_handles[dataset_label] = Line2D(
                [], [],
                color=dataset_color,
                lw=4,
                label=dataset_label,
    ...
    ax.grid(axis="y", alpha=0.3)
    # Match legend style/content to the boxplot: only dataset/model names
    legend_items = list(dataset_handles.values())
    legend_labels = [handle.get_label() for handle in legend_items]
    if legend_items:
        handler_map = {tuple: HandlerTuple(ndivide=None)}
        ax.legend(
            legend_items,
...
def plot_subject_curves(
    ...
    for handle, label in split_handles:
...
def plot_per_subject_bars(
    ...
    for combo in combos:
        dataset_color = model_colors.get(combo, GRADIENT_LEGEND_BASE_COLOR)
        label = f"{combo[0]} | {combo[1]}"
        legend_handles.append(Line2D([], [], color=dataset_color, lw=4, label=label))
        legend_labels.append(label)
    for ax, subject in zip(flat_axes, subjects):
        plotted = False
        x = np.arange(len(SPLIT_ORDER))
        ...
        for idx_combo, combo in enumerate(combos):
            ...
            # Use the same color for all splits for a given dataset
            dataset_color = model_colors.get(combo, GRADIENT_LEGEND_BASE_COLOR)
            colors = [dataset_color] * len(SPLIT_ORDER)
            bars = ax.bar(
                x + offset,
                means,
                width=width * 0.9,
    ...
    fig.suptitle("Per-Subject F1 (Best Validation Epoch)", y=0.99, fontsize=20)
